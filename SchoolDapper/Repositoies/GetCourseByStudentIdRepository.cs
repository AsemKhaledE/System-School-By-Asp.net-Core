...
{
    public class GetCourseByStudentIdRepository : IGetCourseByStudentIdRepository
    {
        private readonly IConfiguration _config;
        private string Connectionstring = "PersonDatabase";

        public GetCourseByStudentIdRepository(IConfiguration config)
        {
            _config = config;
        }
        ...
        public List<StudentCourse> GetListCoursesById(int id)
        {
            try
            {
                using IDbConnection db = new SqlConnection(_config.GetConnectionString(Connectionstring));
                ...
                return db.Query<StudentCourse>($"[dbo].[SelectCourseByStudentID]", dbparams, commandType: CommandType.StoredProcedure).ToList();
            }
            catch (Exception ex)
            {
